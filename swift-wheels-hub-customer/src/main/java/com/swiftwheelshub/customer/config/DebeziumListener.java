...
import com.swiftwheelshub.customer.service.UserProducerService;
...
public class DebeziumListener {
    ...
    public DebeziumListener(Configuration userConnectorConfiguration, UserProducerService userProducerService, ObjectMapper objectMapper) {
    ...
    private void handleChangeEvent(RecordChangeEvent<SourceRecord> sourceRecordRecordChangeEvent) {
        ...
        if (ObjectUtils.isNotEmpty(sourceRecordChangeValue)) {
            ...
            if (Operation.READ != operation) {
                ...
                Map<String, Object> payload = getPayload(struct);

                UserDto userDto = objectMapper.convertValue(payload, UserDto.class);
                processUser(operation, userDto);
    ...
    private Map<String, Object> getPayload(Struct struct) {
    ...
    private void processUser(Operation operation, UserDto userDto) {
        if (Operation.CREATE.equals(operation)) {
            userProducerService.sendSavedUser(userDto);
        }

        if (Operation.UPDATE.equals(operation)) {
            userProducerService.sendUpdatedUser(userDto);
        }
    ...
    private String replaceUnderscoresWithUpperCases(String fieldName, List<Integer> indexes) {
        ...
        for (Integer index : indexes) {
            ...
            updatedFieldName.setCharAt(newIndex, Character.toUpperCase(fieldName.charAt(newIndex)));
        }

        return updatedFieldName.toString().replace(UNDERSCORE, REPLACEMENT);
    }
